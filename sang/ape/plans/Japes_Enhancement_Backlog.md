...
**Status swept 2026-08-25**, item by item against the tree rather than from memory. Six of the ten
were already built, several of them long enough ago that the item's own problem statement had gone
stale and would have led to building the wrong thing (see the closed P2 composition item for the
clearest case). A struck-through heading means closed, with what shipped recorded underneath.
Remaining open: the publish-gate extension, the Studio positioning decision, and the mock harness.
The Studio tool-catalog item is guidance for Studio, not japes work.

---

## ~~P0 — Fix: docs-grounding delivers no content~~

**CLOSED 2026-08-25.** Shipped as `InteractiveAgent._build_read_tools` plus `Skill.reads`,
gated exactly as this item asked: each generated `list_`/`read_`/`search_` tool narrowed through
`admit_hop(f"doc_source:{name}")`, the same `PermissionScope` mechanism skills already used, rather
than a new one. `tests/test_agent_reads.py` covers the empty-source `UNAVAILABLE`, the no-match
search, an id outside the source, the read cap, a KH failure degrading rather than raising, and a
`doc_source:` refusal excluding the tools entirely.

**One acceptance sub-point is not evidently covered:** "a directory torn down before vs. after the
agent call completes". Nothing in that test file exercises teardown ordering. Carried forward as
its own note rather than counted as done.
...
**CLOSED 2026-08-25.** Resolved as option (a): `agents/interactive/reads.py` uses
`jazzx_sdk.tools.build_summary_index` rather than hand-rolling a second index-then-fetch, and its
docstring records that `select_items` is deliberately *not* used and why. The decision this item
asked for was made and is written down where the next reader will find it.
...
**CLOSED 2026-08-25.** Answered by `manifest/spec_binding.py`: `bind_spec` narrows a spec by
its manifest and `build_from_manifest` produces a running agent from the pair. The two schemas are
reconciled by making the relationship explicit (manifest as policy over spec as capability) rather
than by merging them, which is one of the options this item listed.
...
## ~~P1 — Add tiering to `SkillRegistry` / `GuardrailRegistry`~~

**CLOSED 2026-08-25.** Shipped as `_TieredRegistry` (`tier`, `tier_of`, and an override guard
the item did not ask for: a less-trusted tier cannot shadow a more-trusted name without
`allow_override=True`). Reuses `TemplateRegistry`'s idiom exactly as this item proposed. The family
has since gained `freeze()` as well, so a deployment seals its registries before serving.
...
**CLOSED 2026-08-25. Both halves done.** Full record, including this item's original text:
`status/done_skill_spec_ref_composition.md`.

The shape this item proposed shipped essentially verbatim: `Skill.spec_ref` resolved against
`ProfileRegistry`, run by `InteractiveAgent._build_composed_skill_tool` as its own fully-guarded
turn and exposed to the parent via `as_tool`, rather than collapsing to instructions plus tools.
That met the stated acceptance.

**The problem statement had gone stale, and checking it changed what got built.** It named three
fields whose absence flattens a nested assistant. Two were already handled: `spec_ref` resolves a
full `InteractiveAgentSpec`, whose own `guardrails` and `knowledge` apply through the shared
guardrail catalog and the fabric, so putting them on `Skill` would have duplicated what the
referenced spec owns with no rule for which wins. This item predates `spec_ref`, which did not
satisfy those two so much as make them unnecessary.

The third was real but misplaced: `output_schema` was missing from `InteractiveAgentSpec`, not from
`Skill`. It was a constructor argument only, so no spec could declare one in YAML and a composed
assistant, built from a spec, could not have one by any route. Now `spec.output_schema` names a
model in a `SchemaRegistry`, resolved at build time.

Two docstrings claiming all three "apply, not flattened away" were true for two and false for the
third; both corrected.

---

## P2 — Extend `ProfileRegistry.validate()` into the full publish gate

**STILL OPEN, partly built (checked 2026-08-25).** `validate()` exists and does the
reference-resolution half: every skill, guardrail, MCP server and `spec_ref` resolves, aggregated
into one error. Neither of the other two checks this item asked for is there: **(a)** a capability
with side effects having been explicitly opted into, and **(b)** the profile still passing its
evaluation set. Those are the remaining work.
...
**NOT japes work (checked 2026-08-25).** `tools/platform/discovery.py` exists, which is the
thing this item says to build on. The task itself is conditional ("when Studio's tool-catalog UI is
built") and lands in Studio, not here. Keep it as guidance to hand Studio rather than as an open
japes item, so the backlog stops implying we owe code for it.
...
**STILL OPEN, and it is a decision rather than code (checked 2026-08-25).** Partly overtaken:
the v2 extension model is now settled (a domain adds capability as skills and prompts, not code;
registries layer per tenant and freeze before serving), and we set that direction rather than
receiving it. The naming/positioning question this item raises is not answered by that and still
...
**CLOSED 2026-08-25.** Shipped as `scripts/new_assistant_scaffold.py`, which emits a runnable
profile folder plus a harness and a test, matching the acceptance.
...
**STILL OPEN, one piece landed (checked 2026-08-25).** `MockKnowledgeHubClient` is the
knowledge-hub half and now genuinely persists to `data_dir`, so a push-then-pull round trip across
processes works (issue #57). The assistant-API mock and the "by configuration, not copy-paste"
packaging this item actually asks for do not exist; `jazzx-assistant` still hand-rolls its own.
